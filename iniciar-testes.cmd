...
echo No Owlbear, use: http://localhost:5173/manifest.json
echo Para encerrar, pressione Ctrl+C.
echo.

"%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -Command "$ProgressPreference='SilentlyContinue'; $ours=$false; try { $manifest=(Invoke-WebRequest -UseBasicParsing -Uri 'http://localhost:5173/manifest.json' -TimeoutSec 2).Content | ConvertFrom-Json; $ours=($manifest.author -eq 'DemonRider' -and $manifest.action.popover -like '/index.html*') } catch {}; if ($ours) { exit 0 }; if (Get-NetTCPConnection -LocalPort 5173 -State Listen -ErrorAction SilentlyContinue) { exit 2 }; exit 1"
